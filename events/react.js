...
async function execute(message, args) {
  ...
  if (args.length > 1) {
    message.channel.send(
      ":flushed: Invalid invoke of react command! :flushed:\n**TRY** `bella react (word to react with)`"
    );
    return;
...
function isHeterogram(string) {
  ...
  for (let i = 0; i < string.length; i++) {
    ...
    if (
      stringFreq.hasOwnProperty(string.charAt(i)) ||
      !isNaN(string.charAt(i)) ||
      !string.charAt(i).match(/[a-z]/i)
    ) {
      return false;
    } else {
